...
- **Text Cache**: Stored as plain text files extracted from PDF content
- **Highlights**: Stored as `.hlt` JSON files containing user highlight data
- **File naming**: Uses a title-based slug shared across related paper files
  - Convert the paper title to lowercase
  - Replace every non-alphanumeric character with `_`
  - Collapse consecutive underscores into a single `_`
  - Remove leading and trailing underscores
  - Use the resulting slug as the basename for `.json`, `.md`, `.txt`, and `.hlt`
  - Example: `Learning Activation Functions to Improve Deep Neural Networks` -> `learning_activation_functions_to_improve_deep_neural_networks`
